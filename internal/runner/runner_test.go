...
func softStopHandsOverTheLoop(t *testing.T, jobs int, agentScript string, stopAfter time.Duration) {
	repo := testRepo(t)
	set, _ := promptSet(t, "a-review", "b-review", "c-review", "d-review")
...
func TestSoftStopHandsOverTheRestOfTheLoop(t *testing.T) {
	softStopHandsOverTheLoop(t, 0, `echo "RESULT: no-changes"; sleep 0.4`, 500*time.Millisecond)
}
...
func TestSoftStopInParallelModeHandsOverTheQueue(t *testing.T) {
	softStopHandsOverTheLoop(t, 2, `echo "RESULT: no-changes"; sleep 1.2`, 400*time.Millisecond)
}
