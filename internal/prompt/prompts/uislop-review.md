...
Your goal is to find where the interface looks like every other generated interface, and to push it toward something intentional. The tell is not any single choice; it is the cluster of defaults that nobody chose: the same fonts, the same gradients, the same card grid, the same microcopy. A product's UI should be recognizable with the logo removed. This review is about identity and intent, not usability (ux-review), not accessibility conformance (a11y-review), not web loading performance and animation cost (webperf-review), and not code quality (slop-review owns code tells).
