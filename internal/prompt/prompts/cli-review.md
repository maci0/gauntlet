...
- The strongest evidence is running the CLI itself: `--help` on every subcommand, exit codes, behavior when piped/redirected, `NO_COLOR`/`TERM=dumb`. Do not invoke a subcommand that starts a server or waits for connections. If available, use `shellcheck` and `shfmt` for completion and wrapper scripts. Never install tools.
