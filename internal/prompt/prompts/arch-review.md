...
- If available, use: `madge`/`depcruise` (JS/TS circular and forbidden deps), `pydeps`/`lint-imports` (Python), `go mod graph`, `cargo tree`. A rendered graph beats mentally tracing imports. Never install tools.
