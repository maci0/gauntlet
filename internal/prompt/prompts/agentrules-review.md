...
Instructions:
- Fix order: embedded secrets or unsafe instructions > wrong commands or paths that cause agent errors > stale claims about the repo (removed files, renamed APIs) > redundancy and bloat inflating per-session token cost.
...
- Read the whole file set first; most defects are drift between a rule and the current repo, so verify each claim against the code before flagging it. A secret or a command that cannot run is the highest-value catch.
...
When the rule files are extensive
- A repository whose agent rules run to hundreds of lines, or that carries several rule files with real technical content (invariants, an architecture contract, a release protocol, domain conventions no reviewer would guess), is describing subject matter that deserves its own recurring review rather than a one-time reading.
- Say so explicitly when you see it: name the rule file, name the subject that warrants its own review, and state what such a review would look for on every pass. Do not create the file yourself; prompt-review owns that, and it acts on what you write here.
- The bar is content that is checkable and that drifts: a documented invariant, a build or release contract, a layering rule, a data-handling policy. Style preferences and one-line reminders do not warrant a review of their own.
