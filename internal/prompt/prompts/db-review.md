...
- If available, use: `sqlfluff` (SQL lint), `pg_format` (SQL formatting), and saved `EXPLAIN` output or other query-plan artifacts already in the repository. Do not connect to existing databases or run reviewed queries with `EXPLAIN ANALYZE`: it executes the statement and its side effects, even on a local database. Never install tools.
