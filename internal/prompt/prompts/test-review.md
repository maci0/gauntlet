...
- Missing CI integration or test reporting (note only; infra-review owns pipeline wiring)
...
- If available, use: `coverage` (coverage.py), `cargo-llvm-cov`/`cargo-tarpaulin`, `c8`/`nyc` to find untested paths, mutation testing (`mutmut`, `cargo-mutants`, `stryker`) to find tests that assert nothing. Coverage is a map of gaps, not a quality score. Never install tools.
