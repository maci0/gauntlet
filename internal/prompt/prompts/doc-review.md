...
- If available, use: `vale` (prose lint), `markdownlint` (structure), `lychee --offline` (local/file links only; never fetch remote URLs), `codespell`/`typos` (spelling). Never install tools.
