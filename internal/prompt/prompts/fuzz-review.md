...
- If available, use the ecosystem-native harnesses: `cargo-fuzz`/libFuzzer (Rust/C/C++), `afl-fuzz`/`honggfuzz` (AFL++/Honggfuzz), `go test -fuzz` (Go), Atheris (Python library, not a binary), Jazzer (JVM library, not a binary). Prefer extending an existing harness over inventing one. Never install tools.
