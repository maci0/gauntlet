...
func Supported(tool string) bool {
	...
	def, defined := agent.CustomDef(tool)
	return defined && def.Usage != nil && len(def.Usage.Roots) > 0
}
...
type Watcher struct {
	ad      adapter
	dir     string // the review's working directory, as the agent records it
	since   time.Time
...
func New(tool, dir string, since time.Time) *Watcher {
	adaptersMu.RLock()
	ad, ok := adapters[tool]
	adaptersMu.RUnlock()
	if !ok {
		...
		if def, defined := agent.CustomDef(tool); defined && def.Usage != nil {
			if err := RegisterSpec(tool, Spec{
...
func (w *Watcher) poll(onChange func(Sample)) {
	...
	if grew {
		w.sample = Sample{Output: out, Thinking: thinking, Total: total}
	}
	s := w.sample
	w.mu.Unlock()
